...
class _CreateReportScreenState extends State<CreateReportScreen> {
  ...
  final TextEditingController _recognizedAmountController = TextEditingController();
                    ...
                    child: Padding(
                      ...
                      child: DropdownButton<int>(
                        ...
                        items: jobProvider.jobs.map((job) {
                          return DropdownMenuItem(
                            value: job.id,
                            child: Text(job.name, style: TextStyle(fontSize: 14)),
                          );
                        }).toList(),
                      ),
                    ),
                    ...
                    child: Padding(
                      ...
                      child: DropdownButton<int>(
                        ...
                        items: employeeProvider.employees.map((employee) {
                          return DropdownMenuItem(
                            value: employee.id,
                            child: Text(employee.name, style: TextStyle(fontSize: 14)),
                          );
                        }).toList(),
                      ),
                    ),
              ...
              children: [
                    ...
                    onPressed: () {
                      if (selectedDepartmentId != null &&
                        ...
                        reportProvider.createReport(
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text('Ошибка при создании отчета: $error')),
                          );
                        });
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text('Выберите филиал, должность, сотрудника и счет!')),
                        );
                      }
                    },
                ...
                Expanded(
                  child: ElevatedButton(
                    onPressed: () {
                      setState(() {
                        _showViewFields = !_showViewFields; // Переключаем отображение полей
                        if (_showViewFields) {
                          ...
                          _viewFields.add({
                            ...
                            'Дата утверждения а/о': _dateApprovedController.text,
                            'Назначение': _purposeController.text,
                            'Признанная сумма затрат по а/о': _recognizedAmountController.text,
      ...
      child: Row(
        children: [
          ...
          IconButton(
            ...
            onPressed: () {
              _showDeleteDialog(field); // Показываем диалоговое окно для подтверждения удаления
            },
          ),
        ],
      ),
