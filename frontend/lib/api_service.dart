...
class ApiService {
  final String departmentsUrl = 'http://localhost:8081/departments';  // Укажите свой URL бэкенда
  final String departmentUrl = 'http://localhost:8081/department';  // Укажите свой URL бэкенда
  ...
  Future<List<Department>> getDepartments() async {
    ...
    if (response.statusCode == 200) {
      ...
      return data.map((departmentJson) => Department.fromJson(departmentJson)).toList();
    } else {
  ...
  Future<Department> createDepartment(Department department) async {
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        return Department.fromJson(responseData); // Возвращаем созданный департамент
      } else {
  ...
  Future<void> syncDepartments() async {
    ...
    print('Количество департаментов для синхронизации: ${departmentsToSync.length}');
    ...
    for (var department in departmentsToSync) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          print('Ошибка при синхронизации департамента с ID ${department.id}: ${response.statusCode}');
          print('Ответ от сервера: ${response.body}');
...
// Удаление департамента
  Future<void> deleteDepartment(int id) async {
    ...
    if (response.statusCode == 200 || response.statusCode == 204) { // Учитываем 204 как успешный код
      ...
      print('Ошибка при удалении департамента с ID $id: ${response.statusCode}');
      print('Ответ от сервера: ${response.body}');
  ...
  Future<List<Department>> getUnsyncedDepartments() async {
    try {
      final allDepartments = await getDepartments();  // Получаем все департаменты
      return allDepartments.where((department) => !department.synced).toList(); // Отбираем те, что не синхронизированы
    } catch (e) {
