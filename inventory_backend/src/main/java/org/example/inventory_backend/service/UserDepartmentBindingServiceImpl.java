...
import org.example.inventory_backend.mapper.UserDepartmentBindingMapper;
...
import org.example.inventory_backend.repository.UserDepartmentBindingRepository;
...
public class UserDepartmentBindingServiceImpl implements UserDepartmentBindingService {
    ...
    public Long getUserDepartmentId(Long userId) {
        logger.debug("Getting department ID for userId={}", userId);
        ...
        List<UserDepartmentBinding> bindings = bindingRepository.findByUserId(userId);
        if (bindings.isEmpty()) {
            logger.debug("No bindings found for userId={}", userId);
            return null;
        }

        Long departmentId = bindings.get(0).getDepartment().getId();
        logger.debug("Found departmentId={} for userId={}", departmentId, userId);
        return departmentId;
    }
}